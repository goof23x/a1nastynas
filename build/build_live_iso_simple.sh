...
BUILD_DIR="live-build-a1nas-simple"
cd "$PROJECT_ROOT"
rm -rf "$BUILD_DIR"
mkdir "$BUILD_DIR"
cd "$BUILD_DIR"
...
# Configure live-build with proper Ubuntu settings
lb config \
  --mode ubuntu \
  --architectures amd64 \
  --distribution jammy \
...
show_a1nas_banner() {
    echo -e "\033[0;36m"
    figlet "A1NAS" 2>/dev/null || echo "A1NAS"
    echo -e "\033[0m"
